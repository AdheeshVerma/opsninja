...
> = {
  jira: {
    symbol: "◇",
    description: "Turn approved action items into traceable delivery work.",
  },
  slack: {
    symbol: "#",
    description: "Prepare follow-ups for the channels where work happens.",
  },
  calendar: {
    symbol: "◎",
    description: "Keep meeting context linked to calendar events.",
  },
};
...
export default function IntegrationsPage() {
  ...
  const handleConnect = (platform: "jira" | "slack" | "calendar") => {
    ...
    if (platform === "jira") {
      ...
      if (siteUrl) {
        window.open(
          `${apiBase}/api/v1/auth/jira?user_id=${user.user_id}&site_url=${encodeURIComponent(siteUrl)}`,
          "_self",
        );
      }
    } else {
  ...
  const handleDisconnect = async () => {
    if (platformToDisconnect) {
      await disconnect.mutateAsync(
        platformToDisconnect as "jira" | "slack" | "calendar",
      );
      ...
      <section className="max-w-7xl mx-auto px-8 py-12">
        <div className="max-w-2xl mb-8">
          ...
          <p className="text-base text-text-secondary">
            Connect the tools where your team keeps context and carries work
            forward. Ops Ninja always asks before an external action is sent.
          </p>
        </div>
        ...
        {isError && (
          <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg p-4">
            Failed to load integrations. Please check your connection and try
            again.
          </div>
        )}
        ...
        {!isLoading && !isError && integrations && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {integrations.map((integration) => {
              const meta = INTEGRATION_META[integration.platform] ?? {
                symbol: "●",
                description: "",
              };
              ...
              return (
                ...
                >
                  <span
                    className={`inline-flex items-center justify-center w-12 h-12 mb-4 text-2xl ${integration.platform === "jira" ? "text-blue-600" : integration.platform === "slack" ? "text-purple-600" : "text-green-600"}`}
                  >
                    {meta.symbol}
                  </span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="flex items-center text-sm">
                      <i
                        className={`status-dot ${isConnected ? "status-green" : "status-blue"}`}
                      />
                      <span
                        className={
                          isConnected ? "text-accent-green" : "text-brand"
                        }
                      >
                    ...
                    {isConnected ? (
                      <button
                        ...
                        onClick={() =>
                          setPlatformToDisconnect(integration.platform)
                        }
                      >
                        Disconnect
                      </button>
